...
import type { WhistleblowerReport } from "@/lib/whistleblower-store"
...
const NEXT_STATUSES: Record<string, { label: string; value: string }[]> = {
  new:          [{ label: "Mark Under Review", value: "under_review" }, { label: "Close", value: "closed" }],
  under_review: [{ label: "Mark Resolved",     value: "resolved"     }, { label: "Close", value: "closed" }],
  resolved:     [{ label: "Close",             value: "closed"       }],
  closed:       [],
}

function ReportCard({ report, onUpdate }: { report: WhistleblowerReport; onUpdate: (r: WhistleblowerReport) => void }) {
  ...
  async function updateStatus(status: string) {
    setUpdating(true)
    const res = await fetch(`/api/admin/whistleblower/${report.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    })
    if (res.ok) onUpdate({ ...report, status: status as WhistleblowerReport["status"] })
    setUpdating(false)
  }
...
export default function WhistleblowerAdminPage() {
  ...
  const [filter, setFilter] = useState<"all" | WhistleblowerReport["status"]>("all")

  useEffect(() => {
    fetch("/api/admin/whistleblower")
      .then(r => r.json())
      .then(setReports)
      .finally(() => setLoading(false))
  }, [])

  function handleUpdate(updated: WhistleblowerReport) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-3">
        <Shield className="h-6 w-6 text-[#182858]" />
        <div>
          <h1 className="text-xl font-bold text-[#182858]">Whistleblower Reports</h1>
      ...
      <div className="flex gap-2 flex-wrap">
        {(["all", "new", "under_review", "resolved", "closed"] as const).map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
          ...
          >
            {f === "all" ? `All (${reports.length})` : f === "under_review" ? `Under Review (${counts.under_review ?? 0})` : `${STATUS_CONFIG[f].label} (${counts[f] ?? 0})`}
          </button>
        ))}
      </div>
